testthat::test_that("stepwise works", {
  ...
  testthat::expect_true(all(which(apply(out$beta, 2, function(i) all(!is.na(i)))) %in% c(8,9,10)))
  ...
  testthat::expect_true(all(which(apply(out2$beta, 2, function(i) all(!is.na(i)))) %in% c(8,9,10)))
  testthat::expect_true(all(which(apply(out3$beta, 2, function(i) all(!is.na(i)))) %in% c(1,2,3)))
})
