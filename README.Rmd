...
The goal of `WpProj` is to perform Wasserstein projections from the predictive distributions of any model into the   space of predictive distributions of linear models. We utilize L1 penalties to also reduce the complexity of the model space. This package employs the methods as described in Eric Dunipace and Lorenzo Trippa (2020) <arXiv:2012.09999>.
...
This is a basic example running the `WpProj` function on a simulated dataset:
...
dc <- distCompare(models = list("L1" = fit.p2, "BP" = fit.p2.bp),
                  target = list(parameters = post_beta,
                                  predictions = post_mu))
```
```{r example_continued_plot, eval = FALSE}
plot(dc)
```
```{r example_continued_plot_noecho, echo = FALSE}
plot(dc)
```
...
```
```{r r2_plots_noeval, eval = FALSE}
plot(r2.null)
```
```{r r2_plots_noecho, echo = FALSE}
plot(r2.null)
```

We can also examine how the predictions change in the models for individual observations
```{r ridgeplots, eval = FALSE}
ridgePlot(fit.p2, index = 21, minCoef = 0, maxCoef = 10)
```
```{r ridgeplots_noecho, echo = FALSE}
ridgePlot(fit.p2, index = 21, minCoef = 0, maxCoef = 10)
```