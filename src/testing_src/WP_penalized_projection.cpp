...
//[[Rcpp::export]]
SEXP WP_penalized_proj(SEXP X_,
                  SEXP Y_,
                  SEXP theta_,
                  SEXP D_,
                  SEXP family_,
                  ...
                  SEXP opts_)
{
  ...
  if(method(0) != "projection") Rcpp::stop("You are using the projection function and the method is not set to projection.");
  ...
  sufficient_stat(X, Y,
                  ...
                  method,
                  transport_method); 
  //Y will be sorted if using univariate approx, hilbert, or rank
  // Rcout << xty << std::endl;
  // Rcout << xtx << std::endl;
  // Rcout << theta_norm;
  matrix xty_old = xty;
  ...
  if (family(0) == "gaussian")
  {
    // Rcpp::Rcout << groups <<"\n";
    // sleep();
    solver = new oemXTX_gen(xtx, xty_vec, groups, unique_groups,
    ...
    for(int j = 0; j < infm_maxit; j++) {
      ...
      for(int s = 0; s < S; s++) {
        ...
        solver->init_warm_xty(); // still maps to original xty
          //xtx should be updated automatically
        
        //get new solution based on current xty
        res.col(s) = solver->get_beta();
      }// end loop optimizing beta for a given wt matrix
      ...
      mu_update(X, res, theta, mu, method);
