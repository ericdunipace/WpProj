WInftyL1 <- function(X, Y, theta = NULL, penalty = c("none","lasso"), 
                 lambda = numeric(0), 
  ...
  if(penalty != "none") {
    ...
    if(length(lambda) ==1 ) {
      ...
      if(is.null(rqpenargs$alg)) rqpenargs$alg <- 
...
inftyGroupLambda <- function(x, y, groups, tau = 0.5, lambda, intercept = FALSE, 
