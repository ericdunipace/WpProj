WInfL1 <- function(X, Y, theta = NULL, penalty = c("none","lasso", "mcp","scad"), 
  ...
  beta <- inftyGroupLambda(X = Xmat, Y = Y, groups = rep(1:d,s), lambda = lambda,
                           penalty = penalty,
...
inftyGroupLambda <- function(X, Y, groups, lambda, penalty = "lasso",
                                         ...
                                         iter = 100), ...) 
{
  ...
  problem <- lp_prob_winf(X = X, Y = Y, lambda = lambda[1], groups = groups)
  ...
  if(all(lambda == 0)) {
    ...
    for (pos in seq_along(lambda) ) {
      ...
      #                                          ...)
      return_val[[pos]] <- linf_norm(X = X, Y = Y, problem = problem, model = model, deriv_func = deriv_func, thresholder = thresh_fun,
...
linf_norm <- function(X, Y, problem = NULL, model = NULL, deriv_func, thresholder, lambda, groups, solver, gamma = 1.5, opts = NULL, init = NULL, iter = 100, tol = 1e-7) {
  
  d <- ncol(X)
  if(is.null(init)) init <- rep(0,d)
  
  if( is.null(problem)  | is.null(model)) {
    problem <- lp_prob_winf(X = X, Y = Y, lambda = lambda, groups = groups)
...
lp_prob_winf <- function(X, Y, lambda, groups = NULL) {
  ...
  problem <- list(C = c(1, #max bound
                  ...
                  LB = c(-Inf, rep(-Inf, d_length + beta_length), rep(0, ngroups)),
...
# mcp_deriv <- function(x, lambda, gamma) {
#   norm <- sqrt(sum(x^2))
#   rqPen::mcp_deriv(x, lambda, gamma)
# }
# 
# scad_deriv <- function(x, lambda, gamma) {
#   ifelse()
# }
