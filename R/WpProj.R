...
#' 
#' In the function, if \eqn{\mu} is the original prediction from the original model, then we seek to find a new prediction \eqn{\nu} that minimizes the Wasserstein distance between the two. The Wasserstein distance is a generalization of the Euclidean distance, which is the case when \eqn{p=2}. The Wasserstein distance is also known as the Earth Mover's distance. In our function we have argument `power` that corresponds to the \eqn{p} of the equation above. The default `power` is `2.0` but any value greater than or equal to `1.0` is allowed.
#' 
